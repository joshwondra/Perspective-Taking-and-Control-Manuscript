...
m.o_sad1 <- glm(o_sad~sad.angry+CvP.sad+OvCP.sad+CvP.angry+OvCP.angry, family=binomial(link='logit'), data=ptopen)
...
Figure 1 displays subjects' sadness in the open-ended and closed-ended data. 
    ...
    For subjects who read the sad letter, subjects were less likely to report that they felt sad when they remained objective (\Sexpr{}) 
...
# closed-ended plots
box.sad <- with(pt, nojitterbox(sad, f1=ptfactor, f2=emotfactor, ylab='Sad Boxplot'))
box.down <- with(pt, nojitterbox(down, f1=ptfactor, f2=emotfactor, ylab='Down Boxplot'))
